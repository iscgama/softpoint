<?php
    ...
    $sql = "SELECT id_s, origen_tr FROM traspasos WHERE traspason_tr = " . $id;
    $res = $con->query($sql);
    $res->execute();
    ...
    foreach ($res as $a) {
        ...
        $sql4 = "SELECT exist_e FROM existsuc WHERE id_a = " . $ida1 . " AND id_s = " . $id_origen;
        $res4 = $con->query($sql4);
        $res4->execute();


        foreach ($res4 as $a4) {
            $existencias = floatval($a4['exist_e']) - floatval($canta1);

             $sql3 = "UPDATE existsuc SET exist_e = :existencias WHERE id_a = :ida AND id_s = :origen";
             $statement = $con->prepare($sql3);
             $statement->bindParam(':existencias', $existencias);
             $statement->bindParam(':ida', $ida1);
             $statement->bindParam(':origen', $id_origen);
             $statement->execute();
        }
    }
